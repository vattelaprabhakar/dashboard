...
{
    public class MailCampaignController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            try
            {
                ...
                var products = entity.tbl_campaign.Where(m => m.compId == CurrentCompanyId && m.brId == CurrentCompanyBranchId && m.camp_type.ToUpper() == "EMAIL".ToUpper()).ToList();
        ...
        public ActionResult CampaignSchedule()
        {
            ...
            var data1 = entity.tbl_FortuneBasicInfo.Where(m => m.FromDate <= currentdate && vtodate >= currentdate).ToList();
            if (data1 != null)
            {
                foreach (var item in data1)
                {
                    ...
                    if (data2 != null)
                    {
                        ...
                        foreach (var item2 in data2)
                        {
                            ...
                            if (mou.campcategory.ToUpper() == cc.ToUpper() && mou.camptype.ToUpper() == ct.ToUpper())//&& campaignday==currentday
                            {
                            ...
                            if (mou.campcategory.ToUpper() == cc1.ToUpper() && mou.camptype == ct.ToUpper() && campaignday == currentday)
                            {
                                // variables for send email
                                //string product = "";
                                //string leadstatus = "";
                    ...
                    //ViewBag.Javascript = "Email Campaign completed successfully..!";
                  
                }
              
                //  return Content("<script language='javascript' type='text/javascript'>alert('Email Campaign completed successfully!'){   window.location = '/Dashboard/Index'};</script>");
        ...
        public void SendMail(string body, string message, string leademail, string leadname, string product, string status, int? CurrentCompanyId, int? CurrentCompanyBranchId)
        {
            ...
            try
            {
                ...
                var smtp = new SmtpClient
                {
                    ...
                    Credentials = new NetworkCredential("info@fortunesofttech.com", "@@Hyderabad123"),
